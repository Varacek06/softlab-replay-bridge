...
class ReplayTrayApp : ApplicationContext {
    ...
    private void WorkerLoop() {
        while (running) {
            if (!EnsureMidiOpen()) {
                UpdateStatus("Waiting for loopMIDI Port...", Color.Red);
                Thread.Sleep(3000);
                continue;
            }
            try {
                ...
                while (running && (line = nodeProc.StandardOutput.ReadLine()) != null) {
                    ...
                    if (p[0] == "WHEEL") {
                        ...
                        if (p[1] == "READY") UpdateStatus("Connected (Double-click for Settings)", Color.LimeGreen);
                        else if (p[1] == "WAIT_USB") UpdateStatus("Waiting for USB connection...", Color.Orange);
                        else if (p[1] == "BUSY") UpdateStatus("Device is blocked (close Companion/Resolve)", Color.Red);
                        else UpdateStatus("Disconnected, retrying...", Color.Orange);
                    }
                }
            } catch {}
